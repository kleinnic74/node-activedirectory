...
var ActiveDirectory = function(url, baseDN, username, password, defaults) {
  if (this instanceof ActiveDirectory) {
    ...
    objectClasses = _.extend({}, originalObjectClasses, (this.opts || {}).objectClasses || {}, (defaults || {}).objectClasses || {});

    this._isUserResult = function(item) {
       return isUserResult(item, objectClasses.user)
    }
    this._isGroupResult = function(item) {
      return isGroupResult(item, objectClasses.group)
    }
 ...
 * @returns {Boolean}
 */
function isGroupResult(item, groupObjectClass) {
  ...
  if ((item.objectClass) && (item.objectClass.length > 0)) {
    return(_.any(item.objectClass, function(c) { return(c.toLowerCase() === groupObjectClass); }));
  }
  return(false);
}
 ...
 * @returns {Boolean}
 */
function isUserResult(item, userObjectClass) {
  ...
  if ((item.objectClass) && (item.objectClass.length > 0)) {
    return(_.any(item.objectClass, function(c) { return(c.toLowerCase() === userObjectClass); }));
  }
  return(false);
}
      ...
      if (self._isGroupResult(group)) {
        ...
        getGroupMembershipForDN.call(self, opts, group.dn, groups, function(err, nestedGroups) {
          ...
          nestedGroups = _.map(nestedGroups, function(nestedGroup) {
            if (self._isGroupResult(nestedGroup)) {
...
ActiveDirectory.prototype.find = function find(opts, callback) {
  ...
  search.call(self, localOpts, function onFind(err, results) {
    ...
    async.forEach(results, function(item, asyncCallback) {
      ...
      else if (self._isUserResult(item)) {
